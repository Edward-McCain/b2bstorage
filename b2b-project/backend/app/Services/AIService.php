...
Ответ должен быть кратким и на русском языке.";
    ...
    public function generateGeneralRecommendations($userId)
    {
        try {
            ...
            $recommendations = $this->makeOpenAIRequestWithRetry($prompt);
    ...
    /**
     * Выполнить запрос к OpenAI с повторными попытками
     */
    private function makeOpenAIRequestWithRetry($prompt, $maxRetries = 2)
    {
        $attempt = 1;
        
        while ($attempt <= $maxRetries) {
            try {
                Log::info("AIService: Попытка {$attempt} запроса к OpenAI");
                ...
                Log::info("AIService: Получен ответ от OpenAI (попытка {$attempt})");
                
                // Проверяем, что ответ содержит choices
                if (!isset($response->choices) || empty($response->choices)) {
                    throw new \Exception('OpenAI вернул пустой ответ');
                }
                
                $recommendations = $response->choices[0]->message->content;
                
                if (empty($recommendations)) {
                    throw new \Exception('OpenAI вернул пустые рекомендации');
                }
                
                Log::info("AIService: Успешный ответ от OpenAI (попытка {$attempt})");
                return $recommendations;
                
            } catch (\Exception $openaiError) {
                Log::error("AIService: Ошибка OpenAI API (попытка {$attempt}): " . $openaiError->getMessage());
                
                if ($attempt >= $maxRetries) {
                    Log::error("AIService: Все попытки исчерпаны, возвращаем уведомление об ошибке");
                    throw new \Exception('Попробуйте позднее.');
                }
                
                // Ждем перед следующей попыткой (увеличиваем время ожидания с каждой попыткой)
                $waitTime = $attempt * 2; // 2, 4 секунды
                Log::info("AIService: Ожидание {$waitTime} секунд перед следующей попыткой");
                sleep($waitTime);
        ...
        // Если все попытки исчерпаны, возвращаем базовые рекомендации
        return "Рекомендации по управлению запасами:

1. Анализ и оптимизация запасов:
   - Регулярно проводите инвентаризацию
   - Анализируйте товарооборот
   - Оптимизируйте уровни запасов

2. Управление поставками:
   - Установите минимальные уровни запасов
   - Настройте автоматические уведомления
   - Работайте с надежными поставщиками

3. Оптимизация складских процессов:
   - Организуйте эффективное размещение товаров
   - Внедрите систему штрих-кодирования
   - Автоматизируйте процессы учета

4. Мониторинг и контроль:
   - Отслеживайте ключевые показатели
   - Анализируйте отклонения
   - Принимайте своевременные решения

5. Обучение персонала:
   - Обучайте сотрудников работе с системой
   - Проводите регулярные тренинги
   - Внедряйте лучшие практики

6. Анализ эффективности:
   - Измеряйте показатели эффективности
   - Сравнивайте с плановыми значениями
   - Корректируйте стратегии

7. Сотрудничество с отделом продаж:
   - Синхронизируйте планы продаж и закупок
   - Обменивайтесь информацией о спросе
   - Координируйте действия по продвижению";
    }
} 