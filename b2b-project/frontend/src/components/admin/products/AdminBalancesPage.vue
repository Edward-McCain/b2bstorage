    ...
    <div class="bg-white shadow-sm border-b border-gray-200">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="py-6">
          ...
          <p class="mt-2 text-sm text-gray-600">Просмотр складских остатков товаров всех пользователей</p>
        </div>
      </div>
    </div>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    ...
                    <div class="ml-4">
                      ...
                      <div class="text-sm text-gray-500">{{ balance.product?.article }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  <div v-if="balance.product?.user">
                    <div class="font-medium">{{ balance.product.user.user_name || balance.product.user.first_name }}</div>
                    <div class="text-gray-500">{{ balance.product.user.company_name }}</div>
                  </div>
...
import AdminBalanceMovementsModal from './AdminBalanceMovementsModal.vue'
import Multiselect from '@vueform/multiselect'
import '@vueform/multiselect/themes/default.css'

export default {
  name: 'AdminBalancesPage',
  components: {
    AdminLayout,
    AdminBalanceMovementsModal,
    Multiselect
  ...
  setup() {
    ...
    const userOptions = computed(() => {
      return users.value.map(u => ({
        label: u.user_name || u.first_name,
        value: u.id
      }))
    })
    ...
    const loadBalances = async (page = 1) => {
      ...
      try {
        ...
        const response = await api.post('/admin/balances', requestData)
