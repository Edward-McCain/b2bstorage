  ...
  <div class="bg-white rounded-lg shadow-sm p-6 lg:p-8">
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center space-x-3">
        ...
        <div>
          ...
          <h1 class="text-lg lg:text-xl font-bold text-gray-900 mt-1">Удаление инвентаризации</h1>
        </div>
      </div>
      <button
        @click="copyEndpoint"
    ...
    <div class="mb-6">
      ...
      <p class="text-sm lg:text-base text-gray-600 leading-relaxed">
        Удаляет инвентаризацию по указанному идентификатору. Удаление возможно только для 
        инвентаризаций в статусе "draft". При удалении также удаляются все связанные файлы и позиции.
      </p>
    </div>
    ...
    <div class="mb-6">
      ...
      <div class="bg-gray-50 rounded-lg p-4">
        <div class="space-y-3">
          ...
          <div class="flex items-start space-x-3">
            <span class="text-sm font-medium text-gray-700 min-w-24">deleted_items</span>
            <div class="flex-1">
              <span class="text-sm text-gray-600">Количество удаленных позиций</span>
            </div>
          </div>
          <div class="flex items-start space-x-3">
            <span class="text-sm font-medium text-gray-700 min-w-24">deleted_files</span>
            <div class="flex-1">
              <span class="text-sm text-gray-600">Количество удаленных файлов</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center space-x-3 p-3 bg-red-50 rounded-lg">
          ...
          <div>
            ...
            <div class="text-sm text-red-600">Инвентаризация не может быть удалена</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      curlExample: `curl -X DELETE "https://api.example.com/api/inventories/1" \\
  ...
  methods: {
    async copyEndpoint() {
      const endpoint = 'DELETE /api/inventories/{id}'
      try {
