  ...
  <div class="bg-white rounded-lg shadow-sm p-6 lg:p-8">
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center space-x-3">
        <div class="bg-red-100 p-2 rounded-lg">
          <ClipboardList class="h-5 w-5 text-red-600" />
        </div>
        <div>
          ...
          <h1 class="text-lg lg:text-xl font-bold text-gray-900 mt-1">Удаление файла инвентаризации</h1>
        </div>
      </div>
      <button
        @click="copyEndpoint"
    ...
    <div class="mb-6">
      ...
      <p class="text-sm lg:text-base text-gray-600 leading-relaxed">
        Удаляет файл инвентаризации по его уникальному идентификатору. 
        Файл полностью удаляется из системы вместе с физическим файлом на сервере.
      </p>
    </div>
    ...
    <div class="mb-6">
      ...
      <div class="bg-gray-50 rounded-lg p-4">
        <div class="space-y-3">
          ...
          <div class="flex items-start space-x-3">
            <span class="text-sm font-medium text-gray-700 min-w-24">deleted_file</span>
            <div class="flex-1">
              <span class="text-sm text-gray-600">Информация об удаленном файле</span>
            </div>
          </div>
          <div class="flex items-start space-x-3">
            <span class="text-sm font-medium text-gray-700 min-w-24">file_size</span>
            <div class="flex-1">
              <span class="text-sm text-gray-600">Размер удаленного файла в байтах</span>
            </div>
          </div>
          <div class="flex items-start space-x-3">
            <span class="text-sm font-medium text-gray-700 min-w-24">storage_freed</span>
            <div class="flex-1">
              <span class="text-sm text-gray-600">Освобожденное место в хранилище</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center space-x-3 p-3 bg-red-50 rounded-lg">
          ...
          <div>
            ...
            <div class="text-sm text-red-600">Ошибка удаления файла</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  name: 'InventoryFileDeleteMethod',
  components: {
    ClipboardList,
    Copy,
    AlertCircle
  ...
  data() {
    return {
      curlExample: `curl -X DELETE "https://api.example.com/api/inventory-files/1" \\
  ...
  "file_size": 512000,
  "storage_freed": 512000
}`
    }
  },
  methods: {
    async copyEndpoint() {
      const endpoint = 'DELETE /api/inventory-files/{id}'
      try {
