  ...
  <div class="bg-white rounded-lg shadow-sm p-6 lg:p-8">
      ...
      <div class="flex items-center space-x-3">
        ...
        <div>
          <div class="flex items-center space-x-2">
            ...
            <span class="font-mono text-sm lg:text-base text-gray-900">/inventory-files/{id}</span>
          </div>
          <h1 class="text-lg lg:text-xl font-bold text-gray-900 mt-1">Получение файла инвентаризации</h1>
        </div>
      </div>
      <button
        @click="copyEndpoint"
    ...
    <div class="mb-6">
      ...
      <p class="text-sm lg:text-base text-gray-600 leading-relaxed">
        Возвращает информацию о файле инвентаризации по его уникальному идентификатору. 
        Включает метаданные файла и URL для скачивания.
      </p>
    </div>
      ...
      <div class="bg-gray-50 rounded-lg p-4">
        <div class="space-y-3">
          ...
          <div class="flex items-start space-x-3">
            ...
            <div class="flex-1">
              <span class="text-sm text-gray-600">Информация о файле</span>
            </div>
          </div>
          <div class="flex items-start space-x-3">
          ...
          <div class="flex items-start space-x-3">
            ...
            <div class="flex-1">
              <span class="text-sm text-gray-600">ID инвентаризации (null для черновиков)</span>
            </div>
          </div>
          <div class="flex items-start space-x-3">
          ...
          <div class="flex items-start space-x-3">
            ...
            <div class="flex-1">
              <span class="text-sm text-gray-600">Статус черновика (true/false)</span>
            </div>
          </div>
          <div class="flex items-start space-x-3">
            <span class="text-sm font-medium text-gray-700 min-w-24">created_at</span>
            <div class="flex-1">
              <span class="text-sm text-gray-600">Дата создания файла</span>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      curlExample: `curl -X GET "https://api.example.com/api/inventory-files/1" \\
  ...
  methods: {
    async copyEndpoint() {
      const endpoint = 'GET /api/inventory-files/{id}'
      try {
