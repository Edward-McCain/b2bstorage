  ...
  <div class="bg-white rounded-lg shadow-sm p-6 lg:p-8">
      ...
      <div class="flex items-center space-x-3">
        ...
        <div>
          <div class="flex items-center space-x-2">
            ...
            <span class="font-mono text-sm lg:text-base text-gray-900">/receipt-files/{id}</span>
          </div>
          <h1 class="text-lg lg:text-xl font-bold text-gray-900 mt-1">Удаление файла оприходования</h1>
        </div>
      </div>
      <button
        @click="copyEndpoint"
    ...
    <div class="mb-6">
      ...
      <p class="text-sm lg:text-base text-gray-600 leading-relaxed">
        Удаляет файл оприходования по его уникальному идентификатору. Удаление возможно только для файлов, 
        прикрепленных к оприходованиям в статусе "draft". При удалении файл физически удаляется с сервера.
      </p>
    </div>
        ...
        <div class="flex items-start space-x-3">
          ...
          <div class="space-y-2">
            <p class="text-sm text-yellow-800">
              <strong>Удаление возможно только для файлов оприходований в статусе "draft"</strong>
            </p>
            <p class="text-sm text-yellow-700">
              При удалении файла:
            </p>
            <ul class="text-sm text-yellow-700 list-disc list-inside ml-4 space-y-1">
              <li>Файл физически удаляется с сервера</li>
              <li>Запись о файле удаляется из базы данных</li>
              <li>Операция необратима</li>
            </ul>
            <p class="text-sm text-yellow-700 mt-2">
              <strong>Внимание:</strong> Убедитесь, что файл больше не нужен перед удалением!
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      curlExample: `curl -X DELETE "https://api.example.com/api/receipt-files/1" \\
  ...
  methods: {
    async copyEndpoint() {
      const endpoint = 'DELETE /api/receipt-files/{id}'
      try {
