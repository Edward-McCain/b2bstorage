<template>
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h3 class="text-lg font-semibold text-gray-900">POST /transfers/{id}/confirm</h3>
        <p class="text-sm text-gray-600">Подтверждение перемещения</p>
      </div>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        <p class="text-gray-700 text-sm">
          Подтверждает перемещение товаров. Доступно только для черновиков. После подтверждения товары резервируются на складе отправления.
        </p>
      </div>
      ...
      <div>
        ...
        <div class="bg-gray-900 rounded-lg p-4 overflow-x-auto">
          <pre class="text-green-400 text-sm"><code>POST /api/transfers/1/confirm
...
Accept: application/json</code></pre>
  ...
  "message": "Перемещение успешно подтверждено"
}</code></pre>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="space-y-2">
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm text-gray-700">Conflict - Невозможно подтвердить перемещение в текущем статусе</span>
          </div>
          <div class="flex items-center gap-2">
            <div class="w-16 text-center py-1 bg-red-100 text-red-800 text-xs font-medium rounded">
              422
            </div>
            <span class="text-sm text-gray-700">Validation Error - Недостаточно товара на складе</span>
          </div>
        </div>
      </div>
    </div>
