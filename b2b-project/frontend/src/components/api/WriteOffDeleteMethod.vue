  ...
  <div class="bg-white rounded-lg shadow-sm p-6 lg:p-8">
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center space-x-3">
        ...
        <div>
          ...
          <h1 class="text-lg lg:text-xl font-bold text-gray-900 mt-1">Удаление списания</h1>
        </div>
      </div>
      <button
        @click="copyEndpoint"
    ...
    <div class="mb-6">
      ...
      <p class="text-sm lg:text-base text-gray-600 leading-relaxed">
        Удаляет списание по его уникальному идентификатору. Удаление возможно только для списаний 
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center space-x-3 p-3 bg-red-50 rounded-lg">
          ...
          <div>
            ...
            <div class="text-sm text-red-600">Списание не может быть удалено</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      curlExample: `curl -X DELETE "https://api.example.com/api/write-offs/1" \\
  ...
  methods: {
    async copyEndpoint() {
      const endpoint = 'DELETE /api/write-offs/{id}'
      try {
