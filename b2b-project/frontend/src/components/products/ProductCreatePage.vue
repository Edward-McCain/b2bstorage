    ...
    <div class="mb-6 w-full" style="position: sticky;top: 62px;background: #fff;z-index: 99;padding: 10px 0;">
      <div class="flex flex-col gap-3 sm:inline-flex sm:flex-row sm:items-center w-full">
        ...
        <div class="flex gap-2 mt-3 sm:mt-0">
          <button @click="handleSave" :disabled="!product.name || !productId || isSavingDraft || isSavingProduct" class="bg-lime-500 hover:bg-lime-600 text-white font-semibold px-6 py-2 rounded-lg shadow transition text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
...
const product = reactive({
  name: '',
  description: 'Тестовое описание товара для быстрого тестирования функционала',
  category: 'avtomobili-i-aksessuary',
  subcategory: 'avtomobili',
  country: { label: 'Россия', value: 1 },
  supplier: 'ООО "Тестовый поставщик"',
  article: 'ART-001',
  code: 'CODE-123',
  external_code: 'EXT-456',
  unit: { label: 'Штука', value: 'Штука' },
  weight: 1.5,
  volume: 0.5,
  vat: '20%',
  packing: { label: 'Штучная', value: 'Штучная' },
  accounting_type: { label: 'Без специализированного учета', value: 'Без специализированного учета' },
  product_type: { label: 'Не маркируется', value: 'Не маркируется' },
  barcode_type: { label: 'EAN8', value: 'EAN8' },
  barcode: '12345678',
  cash_register_tax: 'ОСН',
  cash_register_type: 'Товар'
})
